                ...
                <div class="p-6">
                    <form action="{{ route('domains.update', $domain) }}" method="POST">
                        ...
                        <div class="mb-6">
                            ...
                            <x-text-input id="name" name="name" type="text" class="mt-1 block w-full" :value="old('name', $domain->name)" required maxlength="100" />
                                ...
                                @foreach(['bg-blue-500', 'bg-green-500', 'bg-red-500', 'bg-orange-500', 'bg-purple-500', 'bg-pink-500'] as $color)
                                    <label class="flex flex-col items-center gap-1 cursor-pointer">
                                        <input type="radio" name="color" value="{{ $color }}"
                                               class="sr-only peer" {{ old('color', $domain->color) === $color ? 'checked' : '' }} required>
